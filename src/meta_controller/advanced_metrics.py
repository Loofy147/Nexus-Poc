...
class AdvancedMetricsCollector:
    ...
    def collect_comprehensive_metrics(self, time_range: str = '5m', step: str = '15s') -> Dict:
        """
    ...
    def _query_prometheus_metrics(self, time_range: str, step: str) -> Dict:
        ...
        for metric_name, query in queries.items():
            try:
                ...
                print(f"Advanced Metrics Collector: Failed to query Prometheus for {metric_name}: {e}")
                results[metric_name] = None

        print(f"Advanced Metrics Collector: Raw metrics queried from Prometheus: {results}")
        return results
    ...
    def _detect_anomalies(self, metrics_data: Dict) -> List[Dict]:
        ...
        if p95_latency is not None and p95_latency > latency_threshold:
            print(f"Advanced Metrics Collector: ANOMALY DETECTED in p95_latency. Value: {p95_latency}, Threshold: {latency_threshold}")
            anomalies.append({
                'metric': 'p95_latency',
                'value': p95_latency,
